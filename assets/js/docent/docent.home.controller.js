...
const takeAPhoto = () => {
  ...
  payload = {
    ...payload,
    annexes: [
      { name: "Photo" + $("#title").val(), mimeType: "png", file: photo },
    ],
  };
  camera.powerOff();
};
...
const registerIncidence = async () => {
  ...
  let selDate =
    $("#incidenceDate").val().split("T")[0] +
    " " +
    $("#incidenceDate").val().split("T")[1];
  ...
  if (response.registered) {
    toastMessage("Inicencia registrada exitosamente").showToast();
    $("#largeModal").modal("hide");
    getAllIncidencesByEmployee();
  } else {
    toastMessage("La incidencia no pudo ser registrada").showToast();
    $("#largeModal").modal("hide");
  }
};
